...
impl SystemOperations for ProcessSystemOperations {
    ...
    async fn rollback(
        ...
        let is_fork = version_channel(&self.config.version) == Some(UpdateChannel::Fork);
        let release_manifest = if is_fork {
            read_release_manifest(&fork::rollback_official_plugins_dir(&self.config)?)?
        } else {
...
fn restore_applied_release(config: &SystemUpdateConfig, cause: OperationError) -> OperationError {
    match rollback_applied_release(config) {
        Ok(()) => cause,
...
fn rollback_applied_release(config: &SystemUpdateConfig) -> Result<(), OperationError> {
    if version_channel(&config.version) == Some(UpdateChannel::Fork) {
        fork::rollback(config)
    } else {
...
impl Drop for AppliedReleaseGuard<'_> {
    fn drop(&mut self) {
        if self.armed
            && let Err(error) = rollback_applied_release(self.config)
        {
