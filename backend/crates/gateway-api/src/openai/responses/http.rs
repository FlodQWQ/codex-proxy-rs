...
///
/// Chat 是 Responses 的兼容投影：请求先在受限大小内解压并转换为 Responses，
/// 中间件始终处理和验证转换后的 Responses wire，最后才投影回 Chat wire。
pub(crate) async fn chat_completions(
    ...
    let maximum_body_bytes = client.snapshot().responses_max_decompressed_body_bytes();
    let plain = match super::request::decompress_request_body(&body, &headers, maximum_body_bytes) {
        Ok(plain) => plain,
        Err(error) => {
            return super::super::error::protocol_error_response(
                StatusCode::BAD_REQUEST,
    ...
    let request = match serde_json::from_slice(plain.as_ref()) {
        Ok(request) => request,
        Err(_) => {
            return super::super::error::protocol_error_response(
                StatusCode::BAD_REQUEST,
    ...
    let (converted, chat) = match chat::convert_request(request) {
    ...
    let middleware_body = match serde_json::to_vec(&converted) {
    ...
    // 正文已解压并转换，不能把旧的传输编码或长度交给重新解码和中间件。
    headers.remove(CONTENT_ENCODING);
    headers.remove(CONTENT_LENGTH);
    let decoded = match decode_request_with_headers(&middleware_body, &headers, maximum_body_bytes)
    {
        Ok(decoded) => decoded,
        Err(error) => {
            return super::super::error::protocol_error_response(
    ...
    execute_prepared_responses_with_format(
        service,
        prepared,
        ResponsesHttpRequest {
            peer_address: connect_info.map(|Extension(ConnectInfo(address))| address),
        ...
        None,
        crate::openai::router::CHAT_COMPLETIONS_PATH,
        Some(chat),
    )
    .await
}
...
async fn execute_prepared_responses_with_format(
    service: OpenAiService,
    prepared: PreparedRootExecution,
    request: ResponsesHttpRequest,
    authorization: Option<Arc<dyn ResponseAuthorization>>,
        ...
        Box::new(move |prepared, request| {
            Box::pin(async move {
                ...
                let started = terminal_service
                    .start_prepared_response(prepared, decoded, transport, endpoint)
                    .await
    ...
    let response = match middleware {
        Ok(response) => {
            ...
            let response = validated_middleware_response(response, validation, stream);
            let response = match chat {
                Some(chat) => chat_middleware_response(response, chat, stream),
                None => response,
            };
            into_http_response(response, expected).await
        }
...
/// 在 Responses 中间件与交付验证完成后，把成功响应投影为 Chat Completions。
///
/// 不能在中间件之前转换响应，否则插件看到的 wire 和 Responses 验证状态机会
/// 被绕过；这里仅改变客户端交付格式，所有会话提交/取消仍委托给内层 body。
fn chat_middleware_response(
    response: MiddlewareResponse,
    options: ChatOptions,
    streaming: bool,
) -> MiddlewareResponse {
    ...
    let (protocol, status, headers, body, envelope) = response.into_parts();
    let stream = streaming.then(|| ChatStream::new(options.clone()));
    let mut response = MiddlewareResponse::new(
        protocol,
        status,
        headers,
        Box::new(ChatCompletionBody {
            inner: Some(body),
            options,
            stream,
            streaming,
...
impl ChatCompletionBody {
    async fn chat_stream_failure(&mut self) -> Result<Option<MiddlewareFrame>, MiddlewareError> {
        if !self.committed {
        ...
        self.pending.push_back(MiddlewareFrame::new(
            Bytes::from_static(
                b"data: {\"error\":{\"type\":\"server_error\",\"code\":\"middleware_failed\",\"message\":\"The gateway could not convert the upstream response.\"}}\n\n",
            ),
...
impl MiddlewareBody for ChatCompletionBody {
    fn next_frame(&mut self) -> BoxFuture<'_, Result<Option<MiddlewareFrame>, MiddlewareError>> {
        Box::pin(async move {
            loop {
                ...
                if !self.streaming {
                    ...
                    let response = chat::complete_response(response, &self.options, &[]);
                    let bytes = serde_json::to_vec(&response)
                        .map(Bytes::from)
                        .map_err(|_| MiddlewareError::InvalidState)?;
                    return Ok(Some(
                        MiddlewareFrame::new(bytes, MiddlewareFraming::JsonDocument, true)
                            .with_transformed(true),
                    ));
                }
                if frame.framing() != MiddlewareFraming::SseEvent {
                    return Err(MiddlewareError::InvalidState);
                }
                ...
                let frames = self
                    ...
                    .as_mut()
                    .ok_or(MiddlewareError::InvalidState)?
                    .convert(vec![bytes.clone()], None);
                if self.stream.as_ref().is_some_and(|stream| stream.failed) {
                    return self.chat_stream_failure().await;
                }
                self.pending.extend(frames.into_iter().map(|frame| {
                    MiddlewareFrame::new(frame, MiddlewareFraming::SseEvent, false)
                        .with_transformed(transformed || !bytes.is_empty())
                }));
                if terminal {
                    // Responses 验证器已确认原生终止帧；ChatStream 只投影 JSON event，
                    // 因此由这里原样交付唯一的 [DONE]。
                    self.pending.push_back(
                        MiddlewareFrame::new(bytes, MiddlewareFraming::SseEvent, true)
                            .with_transformed(transformed),
                    );
                }
            }
    ...
    fn commit_downstream(
        ...
        Box::pin(async move {
            let inner = self.inner.as_mut().ok_or(MiddlewareError::InvalidState)?;
...
fn encode_collected_events(events: &[ProviderEvent]) -> Result<Vec<u8>, ResponseEncodeError> {
    ...
    // Responses 成功终态必须是 object；否则 Chat 投影会在交付边界才失败，
    // 避免把本应在提交前报告的上游错误映射为中间件 502。
    if !response.is_object() {
        return Err(ResponseEncodeError::Serialization);
    }
