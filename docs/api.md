...
账号视图的 `modelDegradation` 数组按当前时间从最近三小时的 OpenAI 使用记录计算，
不修改启用状态或调度权重。每项包含 `status`（`degraded` / `mitigated`）、`requestId`、
`routingScope`、`groupIds`、`sentModel`、`responseModel`、`detectedAt`、`expiresAt`、`recoveredAt`。
以实际发送的模型为比较基准，避免把管理员配置的模型映射误判成上游降智。
仅比较已知 GPT 分级：Astra / Sol / Luna，以及 GPT 4.1、5 至 5.5 的同系列代次与 mini / nano 档位；
不同系列、未知模型以及代次和档位互有高低时不作优劣判断。这是运维告警规则，不是能力测评。

同一账号、同一路由范围与分组集合、同一规范化发送模型的后续成功请求，响应模型不低于发送模型时，
标记为 `mitigated`（降智已缓解）。任何范围仍有未缓解标记时，账号继续显示“已降智”。
失败请求不能证明恢复；缺失模型、无 Token/费用记录、预热和压缩请求不参与判断。
再次降智会重新计时；最后一次降智满三小时且未复发时返回空标记，正常请求不延长到期时间。
状态依赖已落库记录，进程重启不丢失，也无需数据库迁移；删除使用记录会删除对应证据。

账号页直接展示各额度窗口的 `resetAtDisplay`（重置时间），并保留 `quota.refreshedAtDisplay`
（最近查询/观测时间）。这两个时间含义不同，上游未提供时不推算具体重置时间。
...
本地缓存最多 240 秒，提前 60 秒续期；这是基于实测反馈的保守上限，不是上游保证的有效期。
轮次间隔六秒，单次超时二十五秒；旧版没有签发时间或超过此上限的缓存票据会失效并重新采集。
账号之间并行采集，最多八个并发；同一账号的模型串行，全部完成后等待六秒再进入下一轮；每次使用新连接，
实际出口轮换由代理提供方决定。只影响打票，不改变业务流量的传输配置。

任一模型打票返回 HTTP 429 后，该账号所有模型暂停打票一小时；冷却到期才重新尝试，
仍返回 429 则再次冷却一小时。冷却期限独立持久化，重启、凭据刷新或修改代理不会清除。
状态接口的账号对象返回 `rateLimitedUntil`（Unix 秒，无冷却时为 null）。
这只暂停打票，不把 429 写成永久额度事实，也不改变已有票据的有效期。
业务出站仍使用账号原代理。开启后目标账号缺票会被该模型的选择流程排除；
HTTP 和 WebSocket 出站使用已取得票据覆盖 turn-state，不在业务请求里现场打票。
打票请求和业务请求共用当前账号的 Cookie，响应中的 `__cflb`、`__oailb` 按现有凭据存储保存，
遵守域名、路径、Secure 和过期时间；不跨账号共享。先保存 Cookie，再发布新票据；
缺少可回放的路由 Cookie 或 Cookie 保存失败时不发布票据，尝试结果记为 `cookie_error`。
业务请求缺少有效路由 Cookie 时也拒绝单独发送票据。Cookie 只随打票 POST 发送，不发给出口 IP 查询。
...
它使用 Go 1.27.0 的 net/http（与本次对齐的 sub2api 构建版本一致），每次通过匿名管道接收
必要凭据并返回票据与多条 `Set-Cookie`；不在命令行参数、环境变量、临时文件或日志中传递账号令牌或 Cookie。
升级时须同时替换该组件；旧组件不回传 Cookie，没有既存有效 Cookie 时不能完成打票。
可用 `CPR_CODEX_TICKET_PROBE` 指定其他绝对路径；显式指定但不可执行时打票失败，不静默改用其他传输。
未安装组件且未显式指定时保留原生 Rust 传输兼容旧安装，这种模式不保证 TLS 指纹与 sub2api 一致。
状态接口的 `transport` 区分 `go-http1` / `native-http1`，`harvestIdentity` 显示实际打票版本及 UA。

每次采集使用独立 HTTP/1 连接池，先无账号凭据请求上游同源 `/cdn-cgi/trace`，
再在同一池内打票并关闭连接；Go 组件比较底层连接对象，原生传输比较 TCP 四元组，
确认复用后才将 trace 返回的公网 IP 记作该次出口。trace 最多读取 4096 字节、最多等待五秒；
...
- OpenAI 所有凭据有效且 access token 未过期的 OAuth 账号（包括停用账号）每 30 分钟主动复核额度，
  worker 启动后首次立即查询；不改变启用状态，不查询没有该额度接口的 API Key 账号。
  已耗尽账号也会在最早未恢复窗口的 `resetAt + 2 分钟` 到期后
...
- 正常 Responses 请求会解析上游响应的 rate-limit headers，合并进同一 quota 快照并同步状态。Free、
  K12 等套餐共用该状态机，不存在 K12 专属额度路径。
- OpenAI 普通选号依次优先：已绑定且仍可用的会话账号、账号级 5h 已用额度为 0% 到不足 1% 的
  Plus、全局打票开启时选中的打票账号（不限套餐）、其余账号。会话账号优先于后三层，
  其暂时因额度、冷却、并发或请求间隔不可用时才回退；后三层内部保留配置的权重、轮转和会话偏好。
  Plus 到达 1% 后退出最高优先级，不是账号用量上限；在途请求不会中断，可能超过阈值。
  未知、已过期或非 5h 的额度不触发 Plus 优先。权限、禁用、冷却、额度耗尽、并发限制和打票模型的
  有效票据检查仍生效；显式固定账号和原生续写绑定不参与跨账号优先级切换。
